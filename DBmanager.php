...
function workrpnull($jobid){//学生インタビューがあるかないか(ある場合1以上、ない場合" "を返す)
   try {
		$queryset = mysql_query('SELECT COUNT(JOBID) FROM workrp WHERE JOBID ='.$jobid);
			$arr = array();
			while ($data = mysql_fetch_array($queryset)){
			array_push($arr, $data);
			}
...
function comentUpdate($school,$text,$textid){
	...
	$result_flag;
		if($school==1){
		$result_flag = mysql_query("UPDATE studentiv SET IHEAD = 'report[0]',IDATE = 'report[1]', STNAME = 'report[2]', IUPTIME = 'report[3]'IUPNAME = 'report[4]',  WHERE STUDENTID = '$textid'");
		}
		if($school==2){
...
function jobUpdate($jobID,$jobInfo){//職業の更新
...
function sbjctImgResult($tagID){//学生インタビューがあるかないか(ある場合1以上、ない場合" "を返す)
   try {
			$arr = array();
		$tag = tagCheck($tagID);
		if ($tag[2] == 0){
